...
1. cut the freq cvariable

```{r}
# Cut the Freq variable to have 4 levels of comercios (0,1,2,3,4)
			...
			for (cp in postal_codes$label){
				if ((cp %in% temp_cat$cp_origin) == FALSE){
					...
					ggplot() +
						...
						scale_fill_manual(values = c("0 stores" = "#99E4DB", "1 - 3 stores" = "#7DC7BE", "4 - 6 stores" = "#6FB9B0", "7 - 10 stores" = "#61AAA1", "11 stores" = "#539C93", "exact" = "#ebb83b")) +
						geom_text(data = bars_data, aes(x= commercial, y=incomes_per-4, label=paste(round(incomes_per, digits= 0), "%", sep = "")), size=9, colour = "#332f2e", family="Courier", fontface="bold") +
					...
					theme(legend.position = "none", 
						  ...
						  plot.title = element_text(lineheight=.8, family="Courier", size = 20), 
						  panel.grid.major.y = element_line(size = .7, colour = "#c4cac9", linetype = "dashed"), 
